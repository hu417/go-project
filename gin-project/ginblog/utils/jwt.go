...
// 生成token
func (*jwtStr) GenerateJwt(userid int64, username, role string, sign_key string, expires int64) (string, error) {
	// 初始化claim
	claim := &MyCustomClaims{
		...
		RegisteredClaims: jwt.RegisteredClaims{
			...
			ExpiresAt: jwt.NewNumericDate(time.Now().Add(time.Second * time.Duration(expires))), //过期时间,秒
