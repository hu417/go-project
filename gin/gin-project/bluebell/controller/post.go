...
func GetPostDetailHandler(c *gin.Context) {
	...
	// 2. 根据id取出帖子数据（查数据库）
	data, err := logic.GetPostById(c, pid)
	if err != nil {
...
func GetPostListHandler(c *gin.Context) {
	...
	var p *req.ParamPage
	if err := c.ShouldBindJSON(&p); err != nil {
		zap.L().Error("get page detail with invalid param", zap.Error(err))
		resp.ResponseError(c, resp.CodeInvalidParam)
		return
	}

	// 获取数据
	data, err := logic.GetPostList(c, p)
	if err != nil {
...
//	@Param			object			query	req.ParamPostList	false	"查询参数"
//	@Security		ApiKeyAuth
...
func GetPostListHandler2(c *gin.Context) {
	...
	p := &req.ParamPostList{
		Page:  1,
		Size:  10,
		Order: req.OrderTime, // magic string
	}
