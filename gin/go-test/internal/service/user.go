...
type UserService interface {
	...
	GetProfile(ctx context.Context, userId string) (*request.GetProfileResponseData, error)
...
func (s *userService) Register(ctx context.Context, req *request.RegisterRequest) error {
	...
	if err == nil && user != nil {
		return errs.ErrEmailAlreadyUse
	}
...
func (s *userService) GetProfile(ctx context.Context, userId string) (*request.GetProfileResponseData, error) {
	user, err := s.userRepo.GetByID(ctx, userId)
	if err != nil {
		return nil, err
	}

	return &request.GetProfileResponseData{
		UserId:   user.UserId,
