...
func JoinBlackList(claims *MyCustomClaims) (err error) {
	...
	err = global.Redis.SetNX(context.Background(), getBlackListKey(string(claimsJSON)), nowUnix, timer).Err()
	return
}
...
func IsInBlacklist(tokenStr string) (bool, error) {
	// 获取加入黑名单的时间
	joinUnixStr, err := global.Redis.Get(context.Background(), getBlackListKey(tokenStr)).Result()
	if err != nil {
