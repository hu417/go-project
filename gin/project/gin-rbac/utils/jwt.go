...
// 生成token
func GenerateJWT(userid uint, username string, role uint, sign_key string, expires int64) (string, error) {
	// 初始化claim
	claim := &MyClaims{
		UserID:   userid,
		Username: username,
...
*/
func ParseJwt(sign_key string, tokenstr string, options ...jwt.ParserOption) (*MyClaims, error) {
	//解析、验证并返回token。
	str, err := jwt.ParseWithClaims(tokenstr, &MyClaims{}, func(token *jwt.Token) (interface{}, error) {
		return []byte(sign_key), nil
	})

	// 校验 Claims 对象是否有效，基于 exp（过期时间），nbf（不早于），iat（签发时间）等进行判断（如果有这些声明的话）。
	if err == nil && str.Valid {
		if claims, ok := str.Claims.(*MyClaims); ok {
