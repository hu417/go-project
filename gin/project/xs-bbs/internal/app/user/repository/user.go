...
func (u *userRepo) Delete(ctx context.Context, userID string) bool {
...
func (u *userRepo) GetUserByID(ctx context.Context, userID string) (user *model.User, err error) {
	user = new(model.User)
	if err = u.db.WithContext(ctx).Where("user_id = ?", userID).First(user).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			err = errs.ErrUserNotExist
			return
		}
		return
	}
