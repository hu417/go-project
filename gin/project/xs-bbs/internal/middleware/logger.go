...
func GinLogger() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if responStatus > 400 && responStatus <= 499 {
			...
			global.Log.Desugar().Warn("HTTP Warning "+cast.ToString(responStatus), logFields...)
			// zap.L().Warn("HTTP Warning "+cast.ToString(responStatus), logFields...)
		} else if responStatus >= 500 && responStatus <= 599 {
			// 除了内部错误，记录 error
			global.Log.Desugar().Error("HTTP Error "+cast.ToString(responStatus), logFields...)
			// zap.L().Error("HTTP Error "+cast.ToString(responStatus), logFields...)
		} else {
			global.Log.Desugar().Debug("HTTP Access Log", logFields...)
